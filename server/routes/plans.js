...
router.get('/', auth, async (req, res) => {
  try {
    ...
    let query = { userId: req.userId }; // Your auth uses req.userId

    if (status) query.status = status;

    const plans = await Plan.find(query).populate('tasks');
    res.json(plans);
  } catch (error) {
...
// Create new plan (This is your original route for manual plans)
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    if (!plan) {
      return res.status(4.04).json({ message: 'Plan not found' });
    }
